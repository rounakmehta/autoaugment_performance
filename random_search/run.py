...
for epoch in range(args.controller_epochs):
    ...
    print('-> Child accuracy: %.3f (elaspsed time: %ds)' % (accuracy, (toc-tic)), file=(open("log.txt", "a")))
    mem_accuracies.append(accuracy)
    mem_policies.append(subpolicies)
...
print('Best policies found:', file=(open("log.txt", "a")))
print()

print (mem_accuracies)
best_accuracy = max(enumerate(mem_accuracies), key=lambda x: x[1])[0]
print ("Best", best_accuracy)
subpolicies = mem_policies[best_accuracy]
